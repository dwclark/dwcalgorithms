...
(defmethod minimum ((vec comparable-array))
  (with-slots (ary cmp) vec
    (if (< 0 (length ary))
	(let ((min (aref ary 0)))
	  (loop for i from 1 below (length ary)
	     do (if (< (funcall cmp (aref ary i) min) 0)
		    (setf min (aref ary i)))
	     finally (return min)))
	nil)))
...
(defmethod maximum ((vec comparable-array))
  (with-slots (ary cmp) vec
	...
	(let ((max (aref ary 0)))
	  (loop for i from 1 below (length ary)
	     do (if (> (funcall cmp (aref ary i) max) 0)
		    (setf max (aref ary i)))
	     finally (return max)))
	nil)))
...
(defmethod successor ((vec comparable-array) value)
  (with-slots (ary cmp) vec
	...
	(loop with next = nil
	   ...
	   do (if (and (< (funcall cmp value (aref ary i)) 0)
		       (or (null next)
			   (< (funcall cmp (aref ary i) next) 0)))
...
(defmethod successor ((vec sorted-array) value)
  (with-slots (ary cmp) vec
	...
	(let ((idx (binary-search ary value cmp))
	  ...
	  (if (and (not (null idx))
		   (< idx (1- (length ary))))
	      (loop with iter = (1+ idx)
		 while (and (null result) 
			    (< iter (length ary)))
		 do (if (< 0 (funcall cmp (aref ary iter) value))
...
(defmethod predecessor ((vec comparable-array) value)
  (with-slots (ary cmp) vec
	...
	(loop with next = nil
	   for i from 0 below (length ary)
	   do (if (and (< (funcall cmp (aref ary i) value) 0)
		       (or (null next)
			   (< (funcall cmp next (aref ary i)) 0)))
		  (setf next (aref ary i)))
	   finally (return next))
	nil)))
  ...
  (with-slots (ary cmp) vec
	...
	(let ((idx (binary-search ary value cmp))
	  ...
	  (if (and (not (null idx))
		 ...
		 while (and (null result) 
			    (<= 0 iter))
		 do (if (< (funcall cmp (aref ary iter) value) 0)
			(setf result (aref ary iter))
			(incf iter))))
	  result)
	nil)))
